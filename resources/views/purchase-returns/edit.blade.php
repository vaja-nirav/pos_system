...
<div x-data="purchaseReturnEditHandler()" class="max-w-7xl mx-auto">

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit Purchase Return</h1>
    ...
    <form action="{{ route('purchase-returns.update', $return->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <x-card>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div>
                    <label class="block mb-1 text-sm font-medium text-gray-600">Supplier</label>
                    <x-input value="{{ $purchase->supplier->name }}" readonly />
                </div>

                <div>
                    <label class="block mb-1 text-sm font-medium text-gray-600">Invoice No</label>
                    <x-input value="{{ $purchase->invoice_no }}" readonly />
                </div>

                <div>
                    <label class="block mb-1 text-sm font-medium text-gray-600">Return No</label>
                    <x-input value="{{ $return->return_no }}" readonly />
                </div>
        ...
        <x-card class="mt-6">
            ...
            <h3 class="text-base font-semibold text-gray-700 mb-4">Return Items</h3>
                ...
                <table class="w-full">
                    <thead class="bg-gray-100">
                        <tr>
                            ...
                            <th class="p-4 text-left">Purchased Qty</th>
                    ...
                    <tbody>
                        ...
                        {{-- Check if this item was previously returned --}}
                        @php
                            $existingReturnItem = $return->items->firstWhere('product_id', $item->product_id);
                        @endphp
        ...
        <x-card class="mt-6">
            ...
            <div class="mt-6 flex gap-3">
                <x-button type="submit">Update Return</x-button>
...
<script>
function purchaseReturnEditHandler() {

    // Build a map of existing return items keyed by product_id
    const existingItems = @json(
        $return->items->keyBy('product_id')->map(fn($i) => ['qty' => $i->qty, 'price' => $i->price, 'subtotal' => $i->subtotal])
    );

    const purchaseItems = @json(
        $purchase->items->map(fn($i) => ['product_id' => $i->product_id, 'cost_price' => $i->cost_price, 'quantity' => $i->quantity])
    );

    const products = purchaseItems.map(item => {
        const existing = existingItems[item.product_id];
        if (existing) {
            return {
                selected: true,
                qty: existing.qty,
    ...
    return {
        products,
        grandTotal: products.reduce((s, p) => s + Number(p.subtotal), 0),
        selectedCount: products.filter(p => p.selected && p.qty > 0).length,
