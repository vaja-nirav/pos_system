...
<div x-data="purchaseReturnHandler()" class="max-w-7xl mx-auto">

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Create Purchase Return</h1>
            <p class="text-gray-500 text-sm">Return items from Purchase: <strong>{{ $purchase->invoice_no }}</strong></p>
        </div>
    ...
    <form action="{{ route('purchase-returns.store') }}" method="POST">
        ...
        <x-card>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div>
                    <label class="block mb-1 text-sm font-medium text-gray-600">Purchase Date</label>
                    <x-input value="{{ date('d M Y', strtotime($purchase->purchase_date)) }}" readonly />
                </div>
