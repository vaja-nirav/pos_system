...
<div x-data="quotationHandler()" class="max-w-7xl mx-auto pb-10">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-500 text-xs mt-1 font-medium">Generate a professional estimate for your client</p>
        </div>
    ...
    <form action="{{ route('quotations.store') }}" method="POST" class="space-y-6">
        ...
        <x-card class="border border-gray-100 shadow-sm rounded-2xl p-0 !overflow-visible bg-white">
            <div class="p-6 pb-4">
                ...
                <p class="text-gray-400 text-[10px] font-medium">Add products and variations to build your quotation</p>
            </div>
                            ...
                            <tr class="group hover:bg-gray-50/50 transition-all">
                                        ...
                                        <template x-if="item.product_type === 'variation'">
                                            <div class="relative" x-data="{ open: false, search: '' }">
                                                ...
                                                <div x-show="open" @click.away="open = false" x-transition class="absolute left-0 right-0 mt-2 bg-white border border-gray-100 rounded-xl shadow-2xl z-[999] overflow-hidden" style="display: none;">
                                                    <div class="p-2 bg-gray-50 border-b border-gray-50">
                                                        <input type="text" x-model="search" placeholder="Filter..." class="w-full px-3 py-1.5 text-[10px] border border-gray-200 rounded-lg outline-none focus:border-indigo-500 bg-white">
                                                    </div>
                                                    <div class="max-h-40 overflow-y-auto custom-scrollbar">
                                                        <template x-for="(vData, vName) in item.all_variations || {}">
                                                            <button 
                                                                ...
                                                                class="w-full px-4 py-2.5 text-left hover:bg-indigo-50/50 transition-colors flex justify-between items-center group"
                                                            >
                                ...
                                <td class="px-4 py-3 w-24">
                                    <input
                                        ...
                                        :name="'quantity[' + index + ']'"
                                        class="w-full border border-gray-200 bg-white rounded-xl px-3 py-2 text-center focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-bold text-gray-700 text-sm shadow-sm"
                                    >
                                </td>
                                ...
                                <td class="px-4 py-3 w-36">
                                    <div class="relative">
                                        ...
                                        <input
                                            ...
                                            @input="calculate(index)"
                                            :name="'price[' + index + ']'"
                                            class="w-full border border-gray-200 bg-white rounded-xl pl-6 pr-3 py-2 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all font-bold text-gray-700 text-sm shadow-sm"
                                        >
                                    </div>
                                </td>
                                ...
                                {{-- Subtotal --}}
                                <td class="px-4 py-3 w-36">
                                    <div class="text-xs font-bold text-gray-700 text-center py-2" x-text="formatCurrency(item.total)"></div>
                                ...
                                <td class="px-6 py-3 text-center">
                                    <button
                                        ...
                                        class="w-9 h-9 flex items-center justify-center bg-red-50 text-red-400 rounded-lg hover:bg-red-500 hover:text-white transition-all border border-red-100 shadow-sm"
                                    >
            ...
            <div class="p-4 bg-gray-50/50 rounded-b-2xl border-t border-gray-100">
                <button
                    type="button"
                    @click="addRow()"
                    class="px-5 py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center gap-2 group"
                >
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
            ...
            <div class="lg:col-span-2">
                <x-card class="border border-gray-100 shadow-sm rounded-2xl p-6 h-full bg-white">
                    <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1 mb-3">
                        Special Instructions / Terms:
                    </label>
                    <textarea
                        name="note"
                        rows="6"
                        placeholder="Write down any terms..."
                        class="w-full border border-gray-200 bg-gray-50/20 rounded-xl px-5 py-4 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all placeholder:text-gray-300 font-medium text-gray-700 text-xs shadow-inner"
                    ></textarea>
                </x-card>
            </div>
            ...
            <div>
                <x-card class="border border-gray-100 shadow-sm rounded-2xl p-6 space-y-5 bg-white">
                    ...
                    <div class="space-y-1.5 px-2">
                        ...
                        <div class="relative">
                            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 font-bold text-xs">₹</span>
                            <input type="number" x-model="discount" @input="calculateTotals()" name="discount" class="w-full border border-gray-200 bg-gray-50/20 rounded-xl pl-8 pr-4 py-2.5 outline-none font-bold text-gray-700 text-sm focus:border-indigo-400 transition-all">
                        </div>
                    </div>
                    ...
                    <div class="flex justify-between items-center px-5 py-4 bg-indigo-600 rounded-xl shadow-lg shadow-indigo-100">
                        <span class="text-[10px] font-bold text-indigo-100 uppercase tracking-wider">Grand Total</span>
                    ...
                    <button type="submit" class="w-full py-4 bg-indigo-600 text-white rounded-xl font-bold uppercase tracking-wider text-[10px] hover:bg-indigo-700 transition-all shadow-md active:scale-95 group">
                        Save Quotation
                    </button>
                </x-card>
            </div>
