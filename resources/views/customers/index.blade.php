...
<div x-data="{ showImportModal: false, showFilter: false }">
    <div class="flex items-center justify-between mb-10">
        <div>
            ...
            <p class="text-gray-500 text-sm tracking-wide mt-1">Manage your customer base and relationship</p>
        </div>
        ...
        <div class="flex items-center gap-3 relative">
            ...
            @can('create_customers')
                <div 
                    ...
                    style="display: none;"
                >
                    <a href="{{ route('customers.export') }}" class="w-full py-3 bg-indigo-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center hover:bg-indigo-600 transition-all shadow-lg shadow-indigo-100">
        ...
        <x-table>
            <thead class="bg-gray-50/50">
                <tr class="text-left border-b border-gray-100">
                    ...
                    <th class="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">Type</th>
                    ...
                    <tr class="hover:bg-gray-50/30 transition-all group">
                        <td class="px-8 py-5">
                            @if($customer->getFirstMediaUrl('customers'))
                                <img src="{{ $customer->getFirstMediaUrl('customers') }}" class="w-14 h-14 rounded-2xl object-cover shadow-sm border border-gray-100 group-hover:scale-105 transition-transform">
                            @else
                        ...
                        <td class="px-8 py-5">
                            <div class="flex flex-col">
                                <span class="font-bold text-gray-800 text-sm tracking-tight">{{ $customer->name }}</span>
                                <span class="text-[10px] font-black text-indigo-500 uppercase tracking-widest mt-0.5">{{ $customer->email }}</span>
                            </div>
                        </td>
                        <td class="px-8 py-5">
                            <span class="text-xs font-bold text-gray-600">{{ $customer->phone }}</span>
                        </td>
                        <td class="px-8 py-5">
                            ...
                            <div x-data="{ openActionMenu: false, openDeleteModal: false }" class="relative inline-block text-left">
                                ...
                                <div x-show="openActionMenu" @click.away="openActionMenu = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95 translate-y-2" x-transition:enter-end="opacity-100 scale-100 translate-y-0" class="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-100 z-[70] overflow-hidden origin-top-right" style="display: none;">
                                    @can('update_customers')
                                        <a href="{{ route('customers.edit', $customer->id) }}" class="flex items-center gap-3 px-6 py-4">
                                            <span class="font-bold text-gray-700 text-xs">Edit Customer</span>
                                        </a>
                                    @endcan
                                    ...
                                    @can('delete_customers')
                                        <button @click="openDeleteModal = true; openActionMenu = false;" class="w-full flex items-center gap-3 px-6 py-4 text-red-500 transition-all group text-left border-t border-gray-50">
                                            <span class="font-bold text-xs">Delete Customer</span>
                                        </button>
                                    @endcan
                                </div>
                                ...
                                <div x-show="openDeleteModal" class="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-[100]" style="display: none;">
                                    <div @click.away="openDeleteModal = false" x-transition class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-10 text-center">
                                        ...
                                        <p class="text-gray-500 text-sm mb-10 leading-relaxed font-medium">This action will permanently delete <span class="text-gray-800 font-bold">"{{ $customer->name }}"</span> and cannot be undone.</p>
                    ...
                    <tr>
                        <td colspan="6" class="px-8 py-12 text-center">
                            <div class="flex flex-col items-center justify-center text-gray-400">
                                ...
                                <span class="text-xs font-black uppercase tracking-widest">No customers found</span>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    >
        ...
        >
            ...
            <div class="p-10 pb-6 flex items-center justify-between border-b border-gray-50">
                <div>
                    <h2 class="text-3xl font-black text-gray-800 uppercase tracking-tight">Import Customers</h2>
                    <p class="text-gray-500 text-sm mt-1">Bulk upload your customers using Excel or CSV</p>
                </div>
            ...
            <div class="p-10 overflow-y-auto custom-scrollbar">
                <form action="{{ route('customers.import') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
                    @csrf
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <a href="{{ route('customers.sample') }}" class="flex-1 py-5 bg-gray-50 text-gray-600 rounded-2xl font-black text-xs uppercase tracking-[0.2em] hover:bg-gray-100 transition-all text-center border border-gray-100">
                            Download Sample
                        </a>
                    </div>
                </form>
                ...
                <div class="mt-12 space-y-6">
                    ...
                    @php
                        $fields = [
                            'Name' => ['required' => true, 'desc' => 'Full name of the customer'],
