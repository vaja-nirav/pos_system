...
<div x-data="saleReturnEditHandler()" class="max-w-7xl mx-auto">

    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Edit Sale Return</h1>
    ...
    <form action="{{ route('sale-returns.update', $return->id) }}" method="POST">
        @csrf
        @method('PUT')
...
<script>
function saleReturnEditHandler() {
    ...
    const saleItems = @json(
        $sale->items->map(fn($i) => ['product_id' => $i->product_id, 'price' => $i->price, 'quantity' => $i->quantity])
    );
