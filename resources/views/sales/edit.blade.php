...
>
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <div>
            ...
            <p class="text-gray-500 text-sm tracking-wide mt-1">Update existing invoice details</p>
        </div>

        <a href="{{ route('sales.index') }}">
            <x-button class="bg-white text-gray-700 border border-gray-200 hover:bg-gray-50 shadow-sm px-6 rounded-2xl font-bold">
                Back
            </x-button>
        </a>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                ...
                <div>
                    ...
                    <input
                        ...
                        readonly
                        class="w-full border border-gray-100 bg-gray-50 rounded-2xl px-5 py-3.5 outline-none font-black text-gray-400"
                    >
                ...
                <div>
                    ...
                    >
                        ...
                        <option value="Bank Transfer" {{ $sale->payment_type == 'Bank Transfer' ? 'selected' : '' }}>Bank Transfer</option>
                        ...
                        <option value="Other" {{ $sale->payment_type == 'Other' ? 'selected' : '' }}>Other</option>
                    </select>
                </div>
            </div>
...
function saleHandler()
{
    return {
        ...
        addVariant(index, vName)
        {
            ...
            // Check if this variant already exists in another row
            ...
            if (item.variation_name !== '' && item.variation_name !== vName) {
                this.products.splice(index + 1, 0, {
                    product_id: item.product_id,
                    product_type: 'variation',
                    variation_name: vName,
                    all_variations: item.all_variations,
                    quantity: 1,
                    price: vData.price,
                    total: vData.price,
                    stock: vData.opening_stock
                });
                this.calculateTotals();
            } else {
                item.variation_name = vName;
                item.price = vData.price;
                item.stock = vData.opening_stock;
        ...
        setPrice(index)
        {
            ...
            if (productType === 'single') {
                let existingItem = this.products.find((p, i) => 
                    p.product_id === productId && i !== index && p.product_type === 'single'
                );
        ...
        calculateTotals()
        {
            this.subtotal = this.products.reduce((sum, item) => sum + Number(item.total), 0);
            this.grandTotal = Number(this.subtotal) - Number(this.discount) + Number(this.tax);
            this.calculateDue();
        },
